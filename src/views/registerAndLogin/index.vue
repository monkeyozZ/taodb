  ...
  <div class="register">
    ...
    <div class="register_body">
      ...
      <div class="form_box">
        ...
        <router-link to="/passlogin" class="redirect">使用密码登录</router-link>
      </div>
      <div v-transfer-dom class="codeConfirm">
        ...
        @on-confirm="onConfirm">
          <div class="pic_verify_box">
            <input type="tel" maxlength="4" v-model="pic_code" ref="pic_verify" autofocus="autofocus" @focus="clearVerifyErrorMsg" @blur="statistics('输入图片验证码', {from: '登录/注册'})" @click="getFocus">
  ...
  methods: {
    ...
    mobileValidate () {
      if (this.form.mobile.length !== 0) {
        ...
        if (!reg.test(this.form.mobile)) {
          ...
          this.mobileErrorMsg = ''
          this.statistics('输入手机号', {from: '登录/注册'})
          return true
        }
      } else {
        this.mobileErrorMsg = '请输入手机号'
    ...
    codeValidate () {
      if (this.form.code.length !== 0) {
        if (this.form.code.length !== 6) {
          ...
          this.codeErrorMsg = ''
          this.statistics('输入短信验证码', {from: '登录/注册'})
          return true
        }
      } else {
    ...
    showConfirm () {
      ...
      if (this.mobileErrorMsg.length === 0) {
        ...
        this.statistics('打开图片验证码弹窗', {from: '登录/注册'})
        registerAndLoginApi.getVerifyCode(obj).then((res) => {
          if (res.data.code === 0) {
            this.codeUrl = 'data:image/jpeg;base64,' + res.data.data
          }
    ...
    onConfirm () {
      if (this.pic_code.length === 0) {
        ...
        this.loading = true
        this.statistics('提交图片验证码', {from: '登录/注册'})
        registerAndLoginApi.getMessageCode(obj).then((res) => {
          if (res.data.code === 0) {
            ...
            this.statistics('图片验证码错误', {from: '登录/注册'})
            return false
          }
        }).catch((err) => {
          console.log(err)
          ...
          if (this.mobileValidate() && this.codeValidate()) {
            ...
            registerAndLoginApi.checkMessageCode(obj).then((res) => {
              if (res.data.code === 0) {
                ...
                this.statistics('短信验证码错误', {from: '登录/注册'})
